    ...
    <main className="min-h-screen bg-black text-neutral-200 selection:bg-red-900 selection:text-white font-sans overflow-x-hidden pt-32 pb-20">
      ...
      <motion.div 
            ...
            >
                ...
                <span className="text-xs tracking-[0.3em] uppercase text-neutral-500">Recent Works</span>
            </motion.div>
        {/* Page Header */}
        <motion.h1 
          initial={{ opacity: 0, y: 20 }}
        ...
        {collectionsData.map((collection) => (
          <section key={collection.id} className="mb-40 border-t border-neutral-900 pt-16">
            
            {/* Collection Header */}
            <div className="grid md:grid-cols-12 gap-12 mb-16">
              <div className="md:col-span-5">
                <div className="flex items-center gap-4 mb-4">
                   <span className="text-red-600 font-mono text-sm">/// {collection.year}</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {collection.outfits.map((outfit) => (
                <Link key={outfit.id} href={`/collections/${outfit.id}`} className="group block">
